        ...
        def check_availability_and_select(unit_type):
            try:
                if self.type_var.get() == "Overnight Stay":
                    ...
                    checkin, checkout = datetime.now(), datetime.now()
                guests = get_total_guests()
                if guests <= 0: messagebox.showerror('Guests','Total guests must be at least 1'); return
            except: messagebox.showerror('Input Error','Please check dates and guest numbers.'); return
            ...
            if not filtered_units: messagebox.showerror("Unavailable", f"No available {unit_type}s found."); self.room_id_var.set("0"); return
        ...
        def handle_confirm():
            ...
            try:
                ...
                if mode == "Overnight Stay":
                    ...
                    today = date.today()
                    s_h, s_m, s_p = int(start_h.get()), int(start_m.get()), start_p.get()
                    e_h, e_m, e_p = int(end_h.get()), int(end_m.get()), end_p.get()
